...
class CollectorDetailActivity : AppCompatActivity() {
    ...
    private fun observeCollectorData(collectorId: Int) {
        collectorViewModel.getCollectorById(collectorId).observe(this) { collector ->
            if (collector != null) {
                // En lugar de usar data binding, establecemos los valores directamente
                binding.tvAlbumDetailTitle.text = collector.name
                binding.tvEmailCollector.text = collector.email
                binding.tvPhoneCollector.text = collector.telephone
